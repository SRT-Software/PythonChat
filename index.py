        ...
        #             st.markdown(texts[i])
            ...
            with globals()["btn1"] :
                if st.button(label=st.session_state.lists[0], use_container_width=True, key='btn4', on_click=button_callback, args=0):
                    ...
                    st.toast('正在生成提示词', icon='🎈')
            with globals()["btn2"] :
                if st.button(label=st.session_state.lists[1], use_container_width=True, key='btn5', args=1):
                    st.session_state.prompt = st.session_state.lists[1]
                    st.toast('正在生成提示词', icon='🎈')
            with globals()["btn3"]:
                if st.button(label=st.session_state.lists[2], use_container_width=True, key='btn6', args=2):
