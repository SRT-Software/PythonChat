...
st_button = Button(label="SPEAKER", margin=(5, 5, 5, 5), width=200)

st_button.js_on_event("button_click", callback)
# Custom JS to initiate recording
result = streamlit_bokeh_events(bokeh_plot=st_button, key="listen")
# Process events
if result:
    for event in result:
        if event.msg == "recognized":
            transcript = event.dict["recognized"]
            st.write(transcript)
